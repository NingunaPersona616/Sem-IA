...
for i=1:mu
    ...
    sigma(:,i) = 0.1.*rand(D,1);   %Mientras mas grande sea el escalar mayor sera la exploracion, mientras sea menor la explotacion aumentara
end

for t=1:G
    for k=1:lambda  %Se rellena el espacio de los hijos de mu hasta mu+lamba
        %Seleccion
        r1 = randi([1 mu]);
        r2 = r1;
...
%             if randi([0 1])
%                 x(j, mu+k) = x(j, r1);  %Mu+1 es la celda del hijo
...
%         end
        ...
        %Sexual Intermedia
        x(:, mu+k) = (x(:, r1)+ x(:, r2))/2;  %Mu+1 es la celda del hijo
        sigma(:, mu+k) = (sigma(:, r1)+sigma(:, r2))/2;
...
%         end
        ...
        %Mutacion
        r = normrnd(0, sigma(:, mu+k)); %Toma un vector aleatorio de una distribucion normal respecto a las varianzas del hijo
        x(:, mu+k) = x(:, mu+k) + r;    %se suma el vector al hijo
    end
